...
async def async_main(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "random_delay", False):
        import random
        # Check for last run time to ensure spacing
        ...
        if os.path.exists(last_run_file):
            with open(last_run_file, "r") as f:
                try:
                    ...
                    # If it's been less than 25 minutes, wait a bit more
                    if elapsed < 1500:
                        extra_wait = random.randint(300, 600)
                        logger.info(f"Recent run detected ({int(elapsed)}s ago). Adding extra wait: {extra_wait}s")
                        await asyncio.sleep(extra_wait)
                except:
                    pass
        
        delay = random.randint(0, 600)  # 0 to 10 minutes
        logger.info(f"Random delay enabled: sleeping for {delay} seconds...")
...
def main() -> None:
    ...
    parser.add_argument(
        "--random-delay",
        action="store_true",
        help="Wait for a random delay (0-15 mins) before starting",
    )
